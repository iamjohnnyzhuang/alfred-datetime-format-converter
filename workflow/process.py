...
def parse_query_value(query_str):
    ...
    try:
        ...
        if query_str == 'now' or query_str == 'NOW':
            d = datetime.now()
        else:
            # Parse datetime string or timestamp
            try:
                d = datetime.fromtimestamp(float(query_str))
            except ValueError:
                d = datetime.strptime(str(query_str), format)
...
def alfred_items_for_value(value):
    ...
    item_value = int(time.mktime(value.timetuple()))

    results.append(alfred.Item(
        title=str(item_value),
        subtitle=u'transform results',
        attributes={
